...
import { CpqApiError, activateAdminUser, getAdminCredentials } from "@/lib/cpq-api";
...
export async function POST(_request: Request, { params }: RouteContext) {
  ...
  try {
    const result = await activateAdminUser(id, getAdminCredentials());
    ...
    return NextResponse.json(
      { error: "INTERNAL_ERROR", message: "Unable to activate user" },
      { status: 500 }
    );
  }
}
